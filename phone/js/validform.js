...
(function ($, win, undef) {
    ...
    Validform.util = {
        dataType: {
            ...
            "url": /^(\w+:\/\/)?\w+(\.\w+)+.*$/,
            "pn":/^[0-9]{1,}$/,
            "tel":/^(0[0-9]{2,3}\-)?([0-9]{7,8})+(\-[0-9]{1,4})?$|1[0-9]{10}$/,
            "f":/^[1-9]+\d*\.?\d*$/
        },
        ...
        _regcheck: function (datatype, gets, obj, curform) {
            ...
            if (passed) {
                ...
                //规则验证通过后，还需要对绑定有 data-compare-type的进行比较
                if (obj.attr("data-compare-type")) {
                    //获取要对比的$元素
                    var compare_type=obj.attr("data-compare-type");
                    var compare_id = obj.attr("data-compare");
                    var $compare = curform.find('#' + compare_id);
                    if($compare.val()){
                        //被比较的值有值的话
                        if (compare_type == 'lt') {
                            //小于
                            if (obj.val() < $compare.val()) {
                                //如开始时间的值小于 结束时间的值
                                passed = true;
                            } else {
                                passed = false;
                    ...
                    info = obj.attr("errormsg")|| Validform.util.getErrormsg.call(obj, curform, datatype);
                }
            } else {
        ...
        check: function (curform, subpost, bool) {
            ...
            //不是在提交表单时触发的ajax验证;
            if (inputval == this.validform_lastval && !$(this).attr("recheck") && !$(this).attr("data-compare-type") && subpost == "") {
    ...
    Validform.prototype = {
       ...
       /**
         *增加有异步验证的判断 
         */
        checkAjax:function(cb){
            var obj=this;
            if(obj.check()){
                //第一步实时验证通过，进一步验证ajax
                var curform=$(obj.forms);
                //循环遍历有ajaxurl的异步验证属性
                var $inputAjax=curform.find('input[ajaxurl]');
                if($inputAjax.length>0){
                    var validformValid;//临时对象来判断整个表单是否有未通过的情况
                    //说明有ajax验证的，进一步遍历inputajax是否验证通过属性
                   $inputAjax.each(function(){
                        var $input=$(this);
                        //ajaxFlag 有 posting  true false几种可能
                        validformValid=$input[0].validform_valid;
                        if(validformValid!='true'){
                            //如果ajax里面有不是true的，说明ajax请求还没有完全验证
                            return false;//跳出循环
                        }
                    });
                    // console.log('validformValid='+validformValid);
                    if(validformValid=='posting'){
                        //说明还在请求中，不做回调，避免触发其他错误的验证逻辑
                    }else if(validformValid=='true'){
                        //说明全部ajax都通过了
                         cb(true);
                    }else{
                        //验证不通过
